...
DROP POLICY IF EXISTS "own financial forecasts" ON public.financial_forecasts;
CREATE POLICY "own financial forecasts" ON public.financial_forecasts FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
DROP TRIGGER IF EXISTS trg_financial_forecasts_touch ON public.financial_forecasts;
