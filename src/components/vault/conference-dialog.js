...
import { AlertTriangle, CheckCircle2, FileText, GitCompareArrows, Inbox, Loader2, Plus, RotateCcw, Save, XCircle, } from "lucide-react";
import { ReceiptViewerPane } from "./receipt-viewer";
...
function SuggestionHint({ value, onApply }) {
    return (<div className="flex flex-wrap items-center gap-2 pt-1 text-xs text-muted-foreground">
      <span>
...
function NewCategoryPopover({ defaultType, onCreate, }) {
      ...
      <PopoverContent align="end" className="w-72 space-y-3">
        ...
        <div className="space-y-1">
          ...
          <Select value={type} onValueChange={setType}>
            ...
            <SelectContent>
              {Object.entries(transactionTypeLabel).map(([v, l]) => (<SelectItem key={v} value={v}>
                  {l}
                </SelectItem>))}
            </SelectContent>
          </Select>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="button" size="sm" disabled={!name.trim() || saving} onClick={async () => {
            setSaving(true);
...
export function ConferenceDialog(props) {
    const { original, draft, suggested, isDirty, busy, canApprove, preview, statusBadge, categories, profiles, properties, banks, accounts, patchDraft, applySuggestion, onRequestClose, onDiscard, onSave, onApprove, onReject, onArchive, onAnalyze, onCompare, onPreviewError, onCreateCategory, hasExtractedData, } = props;
    const [mobileTab, setMobileTab] = useState("file");
    ...
    const availableAccounts = useMemo(() => {
        let list = accounts;
        if (draft?.bank_id)
            list = list.filter((a) => a.bank_id === draft.bank_id);
        if (draft?.profile_id)
    ...
    const missing = useMemo(() => {
        ...
        if (draft.amount == null || Number(draft.amount) <= 0)
            out.push("Valor");
        if (!draft.recipient_name && !draft.description)
            out.push("Destinatário ou descrição");
        if (!draft.transaction_type)
            out.push("Tipo");
        if (!draft.category_id)
            out.push("Categoria");
        if (!draft.profile_id)
            out.push("Perfil financeiro");
        return out;
    }, [draft]);
    if (!original || !draft) {
    ...
    const suggestionFor = (field) => {
        ...
        if (value == null || value === "")
            return null;
        if (String(value) === String(draft[field] ?? ""))
            return null;
        return value;
    };
    ...
    const blockReason = isDirty
        ...
        : missing.length
            ...
            : null;
    const approveButton = (<Button variant="success" disabled={busy || isDirty || missing.length > 0} onClick={dupScore >= 50 ? undefined : onApprove}>
      <CheckCircle2 className="h-4 w-4"/> Aprovar lançamento
    </Button>);
    return (<Dialog open onOpenChange={(o) => {
            if (!o) {
                if (busy)
                    return;
                onRequestClose();
      ...
      <DialogContent onEscapeKeyDown={(e) => {
            ...
            if (!busy)
                onRequestClose();
        }} onInteractOutside={(e) => e.preventDefault()} className="flex h-[100dvh] w-screen max-w-none flex-col gap-0 overflow-hidden rounded-none p-0 sm:h-[94vh] sm:w-[96vw] sm:max-w-[1600px] sm:rounded-xl">
        {/* ---------- Cabeçalho fixo ---------- */}
        <header className="shrink-0 border-b border-border bg-card px-4 py-3 sm:px-5">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex min-w-0 items-center gap-2 text-xs">
                ...
                {original.ocr_data?.document_type && (<Badge variant="outline" className="h-5 py-0 px-1.5 text-[10px] border-muted-foreground/30 capitalize">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                ...
                {isDirty && (<Badge variant="outline" className="border-primary/50 text-primary">
                    Alterações não salvas
                  </Badge>)}
                {dupScore >= 50 && (<button type="button" onClick={onCompare} className={`flex items-center gap-1 rounded-md border px-2 py-0.5 ${dupScore >= 80 ? "border-destructive/50 bg-destructive/10 text-destructive" : "border-yellow-500/50 bg-yellow-500/10 text-yellow-700"}`}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-1 rounded-lg bg-muted p-1 lg:hidden">
            <button type="button" onClick={() => setMobileTab("file")} className={`rounded-md px-3 py-1.5 text-xs font-medium ${mobileTab === "file" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>
              Comprovante
            </button>
            <button type="button" onClick={() => setMobileTab("form")} className={`rounded-md px-3 py-1.5 text-xs font-medium ${mobileTab === "form" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>
              Dados do lançamento
            </button>
          </div>
        </header>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1.25fr)_minmax(390px,0.75fr)]">
          <div className={`min-h-0 min-w-0 overflow-hidden ${mobileTab === "file" ? "flex" : "hidden"} lg:flex`}>
            <ReceiptViewerPane preview={preview} fileName={original.file_name} fileMime={original.file_mime} busy={busy} analyzeLabel={hasExtractedData ? "Reanalisar com IA" : "Analisar agora"} onAnalyze={onAnalyze} onPreviewError={onPreviewError}/>
          </div>

          <div className={`min-h-0 min-w-0 flex-col overflow-y-auto bg-background px-4 py-4 pb-10 ${mobileTab === "form" ? "flex" : "hidden"} lg:flex`}>
            <div className="space-y-4">
              {original.ocr_status === "failed" && (<div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
                  ...
                  <Button size="sm" variant="outline" className="h-8 text-xs border-destructive/30 text-destructive hover:bg-destructive/10" onClick={onAnalyze} disabled={busy}>
                    {busy ? <Loader2 className="mr-2 h-3 w-3 animate-spin"/> : <RotateCcw className="mr-2 h-3 w-3"/>}
                    Tentar análise novamente
                  </Button>
                </div>)}

              {original.ai_confidence && original.ocr_status === "done" && (<div className={`rounded-xl border p-4 ${original.ai_confidence === "ALTA" ? "border-success/30 bg-success/5" :
                original.ai_confidence === "MEDIA" ? "border-yellow-500/30 bg-yellow-500/5" :
                  ...
                  {original.ai_history_summary && (<div className="mt-3 pt-3 border-t border-border/50 text-xs text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
                      <span>• {original.ai_history_summary.count} ocorrências no histórico</span>
                      {original.ai_history_summary.avgAmount && (<span>• Média de {currencyBRL(original.ai_history_summary.avgAmount)}</span>)}
                    </div>)}
                </div>)}


              {isDirty && (<div className="rounded-lg border border-primary/40 bg-primary/5 px-3 py-2 text-xs text-primary">
              ...
              <Section title="Dados principais">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-1">
                    ...
                    {suggestionFor("payment_date") && (<SuggestionHint value={String(suggested.payment_date)} onApply={() => applySuggestion("payment_date")}/>)}
                  </div>
                  <div className="space-y-1">
                    <Label>Valor</Label>
                    <Input type="number" step="0.01" value={draft.amount ?? ""} onChange={(e) => patchDraft({
            amount: e.target.value === "" ? null : Number(e.target.value),
                ...
                <div className="space-y-1">
                  ...
                  {suggestionFor("recipient_name") && (<SuggestionHint value={String(suggested.recipient_name)} onApply={() => applySuggestion("recipient_name")}/>)}
                </div>
                <div className="space-y-1">
                  ...
                  {suggestionFor("recipient_tax_id") && (<SuggestionHint value={String(suggested.recipient_tax_id)} onApply={() => applySuggestion("recipient_tax_id")}/>)}
                </div>
              </Section>
              ...
              <Section title="Pagamento">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-1">
                    ...
                    <Select value={draft.payment_method ?? undefined} onValueChange={(v) => patchDraft({ payment_method: v })}>
                      ...
                      <SelectContent>
                        {Object.entries(paymentMethodLabel).map(([v, l]) => (<SelectItem key={v} value={v}>
                            {l}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
                    {suggestionFor("payment_method") && (<SuggestionHint value={paymentMethodLabel[suggested.payment_method] ?? String(suggested.payment_method)} onApply={() => applySuggestion("payment_method")}/>)}
                  </div>
                  <div className="space-y-1">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-1">
                    ...
                    <Select value={draft.bank_id ?? "none"} onValueChange={(v) => {
            ...
            patchDraft({
                ...
                ...(compatible ? {} : { account_id: null }),
                      ...
                      <SelectContent>
                        ...
                        {banks.map((b) => (<SelectItem key={b.id} value={b.id}>
                            {b.name}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="space-y-1">
                    ...
                    <Select value={draft.account_id ?? "none"} onValueChange={(v) => patchDraft({ account_id: v === "none" ? null : v })}>
                      ...
                      <SelectContent>
                        ...
                        {availableAccounts.map((a) => (<SelectItem key={a.id} value={a.id}>
                            {a.nickname}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
                    {availableAccounts.length === 0 && (<p className="pt-1 text-xs text-muted-foreground">
              ...
              <Section title="Classificação">
                <div className="space-y-1">
                  ...
                  {suggestionFor("transaction_type") && (<SuggestionHint value={transactionTypeLabel[suggested.transaction_type] ?? String(suggested.transaction_type)} onApply={() => applySuggestion("transaction_type")}/>)}
                </div>
                <div className="space-y-1">
                  ...
                  {suggestionFor("category_id") && (<SuggestionHint value={categories.find((c) => c.id === suggested.category_id)?.name ??
                "Categoria sugerida"} onApply={() => applySuggestion("category_id")}/>)}
                </div>
                <div className="space-y-1">
              ...
              <Section title="Vínculos do Meu Cofre">
                <div className="space-y-1">
                  ...
                  <Select value={draft.profile_id ?? undefined} onValueChange={(v) => {
            ...
            patchDraft({ profile_id: v, ...(keepProperty ? {} : { property_id: null }) });
                    ...
                    <SelectContent>
                      {profiles.map((p) => (<SelectItem key={p.id} value={p.id}>
                          {p.name}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-1">
                  ...
                  <Select value={draft.property_id ?? "none"} onValueChange={(v) => patchDraft({ property_id: v === "none" ? null : v })}>
                    ...
                    <SelectContent>
                      <SelectItem value="none">Nenhum</SelectItem>
                      {availableProperties.map((p) => (<SelectItem key={p.id} value={p.id}>
                          {p.name}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                  {suggestionFor("property_id") && (<SuggestionHint value={properties.find((p) => p.id === suggested.property_id)?.name ??
                "Imóvel sugerido"} onApply={() => applySuggestion("property_id")}/>)}
                </div>
              </Section>
            </div>
          </div>
