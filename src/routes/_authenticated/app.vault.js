...
import { CheckCircle2, XCircle, AlertTriangle, Search, Loader2, Inbox, Copy, Archive, Trash2, GitCompareArrows, } from "lucide-react";
import { toast } from "sonner";
import { useServerFn } from "@tanstack/react-start";
import { approveReceipt, rejectReceipt, bulkReceiptAction, bulkUpdateReceipts, deleteReceipts, analyzeReceipt, updateReceiptConference, archiveReceipt, } from "@/lib/receipts.functions";
...
function normalizePaymentValue(value) {
    ...
    if (!text)
        return null;
    if (text.includes("pix") || text.includes("e2e"))
        return "pix";
    if (text.includes("boleto") || text.includes("codigo de barras"))
        return "boleto";
    if (text.includes("ted"))
        return "ted";
    if (text.includes("deb"))
        return "debito";
    if (text.includes("parcel"))
        return "credito_parcelado";
    if (text.includes("cred"))
        return "credito_vista";
    if (text.includes("dinheiro") || text.includes("especie"))
        return "dinheiro";
    if (text.includes("transf") || text.includes("doc"))
...
function normalizeTransactionValue(value) {
    ...
    if (text.includes("fix"))
        return "gasto_fixo";
    if (text.includes("vari"))
        return "gasto_variavel";
    if (text.includes("pessoal"))
        return "pessoal";
    if (text.includes("empresa"))
        return "empresarial";
    if (text.includes("patrim"))
        return "patrimonial";
    if (text.includes("desp"))
        return "despesa";
    return null;
}
function deepFind(source, aliases, depth = 0) {
...
function buildAutoDescription(receipt) {
    ...
    const by = receipt.bank_name
        ...
        : receipt.payment_method
            ? `via ${paymentMethodLabel[receipt.payment_method] ?? receipt.payment_method}`
            : null;
    if (recipient)
    ...
    if (by)
        parts.push(by);
    if (parts.length)
        return `Pagamento ${parts.join(", ")}.`;
    return receipt.file_name ? `Comprovante ${receipt.file_name}` : null;
}
function hydrateReceiptForConference(receipt, categories = [], properties = []) {
    ...
    hydrated.auth_code =
        hydrated.auth_code ??
            firstText(deepFind(ocr, ["auth_code", "codigo_autenticacao", "autenticacao", "id_transacao", "e2e"]));
...
function getStoragePath(receipt) {
...
function hasExtractedConferenceData(receipt) {
    return Boolean(receipt.payment_date ||
        receipt.amount != null ||
...
function dupScoreBadge(score) {
    const s = Number(score ?? 0);
    if (s >= 80)
        return (<Badge className="gap-1 bg-destructive text-destructive-foreground hover:bg-destructive">
        <AlertTriangle className="h-3 w-3"/> Alta {s}
      </Badge>);
    if (s >= 50)
        return (<Badge className="gap-1 bg-yellow-500 text-white hover:bg-yellow-500">
...
function VaultPage() {
    ...
    const [quick, setQuick] = useState("pending");
    ...
    const properties = useQuery({
        ...
        queryFn: async () => (await supabase.from("properties").select("id, name, profile_id").order("name")).data ?? [],
    });
    ...
    const receipts = useQuery({
        ...
        queryFn: async () => {
            ...
            if (quick === "pending")
                qb = qb.in("status", ["pending", "duplicate"]).or(`ocr_status.eq.failed,status.eq.pending`);
            else if (quick === "approved")
                qb = qb.eq("status", "approved");
            else if (quick === "rejected")
                qb = qb.eq("status", "rejected");
            else if (quick === "archived")
                qb = qb.eq("status", "archived");
            else if (quick === "suspected")
                qb = qb.gte("duplicate_score", 50);
            else if (quick === "high_dup")
                qb = qb.gte("duplicate_score", 80);
            if (profileId !== "all")
                qb = qb.eq("profile_id", profileId);
            if (bankId !== "all")
                qb = qb.eq("bank_id", bankId);
            if (categoryId !== "all")
    ...
    const doBulkPatch = async (patch, label) => {
        if (selectedIds.size === 0)
            return;
        setBusy(true);
        try {
    ...
    const doBulkDelete = async () => {
        ...
        try {
            ...
            if (res.storageWarning) {
                ...
                toast.success(`${res.count} lançamento(s) excluídos` +
    ...
    const saveDraft = async () => {
        ...
        try {
            ...
            const res = await saveConference({ data: { receiptId: original.id, patch } });
    ...
    const createCategoryByName = async (name, type) => {
        ...
        const { data, error } = await supabase
            .from("categories")
            .insert({ user_id: userId, name, default_type: type })
            .select("id, name")
            .single();
        if (error || !data) {
    ...
    const analyzeCurrentReceipt = async () => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(res.error ?? "Não foi possível analisar o comprovante");
    ...
    const goToNextPending = async (currentId) => {
        let qb = supabase
            .from("receipts")
            .select("*")
            .eq("status", "pending")
        ...
        if (error || !data) {
            closeEditing();
            if (!error)
                toast.info("Nenhum comprovante pendente restante.");
            return;
        }
        await openEdit(data);
    };
    ...
    const rejectCurrentReceipt = async (note) => {
        if (!original)
            return;
        setBusy(true);
        try {
    ...
    return (<div className="space-y-6 pb-24">
      ...
      <Tabs value={quick} onValueChange={(v) => setQuick(v)}>
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <Select value={profileId} onValueChange={setProfileId}>
            ...
            <SelectContent>
              ...
              {(profiles.data ?? []).map((p) => (<SelectItem key={p.id} value={p.id}>
                  {p.name}
                </SelectItem>))}
            </SelectContent>
          </Select>
          <Select value={bankId} onValueChange={setBankId}>
            ...
            <SelectContent>
              ...
              {(banks.data ?? []).map((p) => (<SelectItem key={p.id} value={p.id}>
                  {p.name}
                </SelectItem>))}
            </SelectContent>
          </Select>
          <Select value={categoryId} onValueChange={setCategoryId}>
            ...
            <SelectContent>
              ...
              {(categories.data ?? []).map((p) => (<SelectItem key={p.id} value={p.id}>
                  {p.name}
                </SelectItem>))}
            </SelectContent>
          </Select>
      ...
      {receipts.isLoading ? (<Card className="p-10 text-center">
          <Loader2 className="mx-auto h-6 w-6 animate-spin text-muted-foreground"/>
        </Card>) : receipts.isError ? (<Card className="p-10 text-center">
          <p className="text-sm font-medium">Não foi possível carregar os comprovantes</p>
          <p className="mx-auto mt-1 max-w-md text-xs text-muted-foreground">
            {receipts.error?.message ?? "Erro inesperado ao consultar o Cofre."}
          </p>
          <Button className="mt-4" variant="outline" onClick={() => receipts.refetch()}>
            Tentar novamente
          </Button>
        </Card>) : filtered.length === 0 ? (<Card className="p-10 text-center">
          ...
          <Card className="hidden md:block overflow-hidden">
            <div className="overflow-x-auto">
              <Table>
                ...
                const highlight = r.duplicate_score >= 80
                    ? "bg-destructive/5"
                    : r.duplicate_score >= 50
                        ? "bg-yellow-500/5"
                        : "";
                return (<TableRow key={r.id} className={`${highlight} cursor-pointer`} onClick={() => openEdit(r)}>
                        ...
                        <TableCell className="text-xs">
                          {banks.data?.find((b) => b.id === r.bank_id)?.name ?? r.bank_name ?? "—"}
                        </TableCell>
                        <TableCell className="text-xs">
                          {profiles.data?.find((p) => p.id === r.profile_id)?.name ?? "—"}
                        </TableCell>
                        <TableCell className="text-xs">{categories.data?.find((c) => c.id === r.category_id)?.name ?? "—"}</TableCell>
                        <TableCell className="text-xs">
                          {r.transaction_type
                        ...
                        <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex justify-end gap-1">
                            {r.duplicate_of && (<Button variant="ghost" size="sm" onClick={() => setCompareId(r.id)} title="Comparar duplicado">
            ...
            {filtered.map((r) => {
                ...
                const highlight = r.duplicate_score >= 80
                    ...
                    : r.duplicate_score >= 50
                        ...
                        : "";
                return (<Card key={r.id} className={`p-3 ${highlight}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <button onClick={() => openEdit(r)} className="flex-1 min-w-0 text-left">
                      ...
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                        {dateBR(r.payment_date)} • {banks.data?.find((b) => b.id === r.bank_id)?.name ?? r.bank_name ?? "—"} •{" "}
                    ...
                    {r.duplicate_of && (<Button variant="ghost" size="icon" onClick={() => setCompareId(r.id)}>
                        <GitCompareArrows className="h-4 w-4"/>
                      </Button>)}
                  </div>
                </Card>);
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pb-2">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" disabled={page === 0 || receipts.isFetching} onClick={() => setPage((p) => Math.max(0, p - 1))}>
                Anterior
              </Button>
              <Button variant="outline" size="sm" disabled={page + 1 >= totalPages || receipts.isFetching} onClick={() => setPage((p) => p + 1)}>
                Próxima
              </Button>
            </div>
          </div>
      ...
      {/* Sticky bulk toolbar */}
      {selectedIds.size > 0 && (<div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 p-3 backdrop-blur md:left-64">
          <div className="mx-auto flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {canBulk && (<BulkFieldSelect label="Tipo" placeholder="Alterar tipo" options={Object.entries(transactionTypeLabel).map(([v, l]) => ({
                    value: v,
                    label: l,
                }))} onPick={(v) => doBulkPatch({ transaction_type: v }, "Tipo")} disabled={busy}/>)}
            </div>
          </div>
        </div>)}
      ...
      {original && draft && (<ConferenceDialog original={original} draft={draft} suggested={suggested} isDirty={isDirty} busy={busy} canApprove={canApprove} preview={preview} statusBadge={statusBadge} categories={categories.data ?? []} profiles={profiles.data ?? []} properties={properties.data ?? []} banks={banks.data ?? []} accounts={accounts.data ?? []} hasExtractedData={hasExtractedConferenceData(original)} patchDraft={patchDraft} applySuggestion={applySuggestion} onRequestClose={requestClose} onDiscard={discardDraft} onSave={saveDraft} onApprove={approveCurrentReceipt} onReject={(note) => {
                setRejectNote(note);
            ...
            }} onAnalyze={analyzeCurrentReceipt} onCompare={() => setCompareId(original.id)} onPreviewError={(message) => setPreview((prev) => ({ ...prev, error: message }))} onCreateCategory={createCategoryByName}/>)}
...
function BulkConfirm({ label, icon: Icon, tone, count, onConfirm, disabled, destructive, }) {
...
function BulkFieldSelect({ label, placeholder, options, onPick, disabled, }) {
...
function CompareDialog({ receiptId, onClose, onChanged, }) {
    const approve = useServerFn(approveReceipt);
    const reject = useServerFn(rejectReceipt);
    const bulkAction = useServerFn(bulkReceiptAction);
    const bulkDelete = useServerFn(deleteReceipts);
    ...
    const reason = useMemo(() => {
        ...
        const sameRecipient = n.recipient_name &&
            o.recipient_name &&
    ...
    return (<Dialog open={!!receiptId} onOpenChange={(o) => {
            if (!o)
                onClose();
        }}>
      <DialogContent className="max-w-6xl">
        ...
        {query.isLoading ? (<div className="grid h-64 place-items-center">
            <Loader2 className="h-6 w-6 animate-spin"/>
          </div>) : !data ? (<p className="text-sm text-muted-foreground">
            Não foi possível carregar os comprovantes.
          </p>) : (<>
            {reason && (<div className="flex items-start gap-2 rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-3 text-sm">
                <AlertTriangle className="mt-0.5 h-4 w-4 text-yellow-600"/>
                <span>{reason}</span>
              </div>)}
            <div className="grid gap-4 md:grid-cols-2">
              <ReceiptPanel title="Comprovante novo" rec={data.newRec} url={data.newUrl} tone="new"/>
              {data.oldRec ? (<ReceiptPanel title="Comprovante existente" rec={data.oldRec} url={data.oldUrl} tone="old"/>) : (<Card className="grid place-items-center p-8 text-sm text-muted-foreground">
                  Nenhum comprovante existente vinculado.
                </Card>)}
            </div>
            <div className="mt-2 flex flex-wrap justify-end gap-2">
              <Button variant="outline" size="sm" onClick={() => run(() => bulkAction({ data: { receiptIds: [data.newRec.id], action: "approve" } }), "Marcado como novo e aprovado")}>
                Manter como novo
              </Button>
              <Button variant="outline" size="sm" onClick={() => run(() => reject({ data: { receiptId: data.newRec.id, reason: "duplicate" } }), "Novo marcado como duplicado")}>
              ...
              <Button variant="outline" size="sm" onClick={() => run(() => bulkAction({ data: { receiptIds: [data.newRec.id], action: "archive" } }), "Novo arquivado")}>
                Arquivar novo
              </Button>
              <Button variant="destructive" size="sm" onClick={() => run(() => reject({ data: { receiptId: data.newRec.id, reason: "rejected" } }), "Novo rejeitado")}>
                Rejeitar novo
              </Button>
              <Button variant="success" size="sm" onClick={() => run(() => approve({ data: { receiptId: data.newRec.id } }), "Novo aprovado mesmo assim")}>
                Aprovar mesmo assim
              </Button>
            </div>
          </>)}
...
function ReceiptPanel({ title, rec, url, tone, }) {
    return (<Card className={`p-3 ${tone === "new" ? "border-primary/50" : "border-muted"}`}>
      ...
      <div className="mb-3 h-64 overflow-hidden rounded border border-border bg-muted/40">
        {url ? (rec.file_mime?.startsWith("image/") ? (<img src={url} alt={title} className="h-full w-full object-contain"/>) : (<iframe src={url} title={title} className="h-full w-full"/>)) : (<div className="grid h-full place-items-center text-xs text-muted-foreground">
            Sem prévia
          </div>)}
      </div>
      <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
        ...
        <dd>
          {rec.transaction_type
            ? transactionTypeLabel[rec.transaction_type]
            : "—"}
        </dd>
      </dl>
    </Card>);
