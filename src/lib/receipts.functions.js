...
async function logAudit(supabase, userId, params) {
    try {
...
export const analyzeReceipt = createServerFn({ method: "POST" })
    ...
    if (!rec.file_path)
        throw new Error("Este lançamento não possui comprovante anexado.");
    ...
    const withoutAccents = (value) => value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    const normalizePaymentMethod = (value) => {
        const text = normalizeString(value);
        if (!text)
            return null;
        const normalized = withoutAccents(text);
        if (normalized.includes("pix") || normalized.includes("e2e"))
            return "pix";
        if (normalized.includes("boleto") || normalized.includes("codigo de barras"))
            return "boleto";
        ...
        if (normalized.includes("dinheiro") || normalized.includes("especie"))
            return "dinheiro";
        if (normalized.includes("transf") || normalized.includes("doc"))
    ...
    let ai_suggested_profile_id = rec.profile_id; // Default to current profile
    let ai_confidence = "NAO_IDENTIFICADO";
    let ai_reason = "";
    ...
    if (extracted.recipient_name) {
        ...
        if (historySummary && historySummary.count > 0) {
            ...
            const sortedCats = Object.entries(historySummary.categories)
                .sort((a, b) => b[1] - a[1]);
            const mostFreqCat = sortedCats[0];
            // Find most frequent profile
            const sortedProfs = Object.entries(historySummary.profiles)
                .sort((a, b) => b[1] - a[1]);
            const mostFreqProf = sortedProfs[0];
            if (mostFreqCat)
                ai_suggested_category_id = mostFreqCat[0];
            if (mostFreqProf)
                ai_suggested_profile_id = mostFreqProf[0];
            // Scoring Confidence
    ...
    if (extracted.recipient_name) {
        ...
        if (existing && existing.length > 0) {
            const r = existing[0];
            recipient_id = r.id;
            if (!ai_suggested_category_id && r.default_category_id)
                ai_suggested_category_id = r.default_category_id;
    ...
    if (!duplicate_of && extracted.amount && extracted.payment_date) {
        ...
        if (sameVD?.length) {
            ...
            for (const d of sameVD) {
                if (extracted.recipient_name && d.recipient_name && d.recipient_name.toLowerCase() === extracted.recipient_name.toLowerCase())
                    s += 15;
                if (extracted.bank_name && d.bank_name && d.bank_name.toLowerCase() === extracted.bank_name.toLowerCase())
                    s += 10;
            }
            score = Math.min(s, 75);
        }
...
export const approveReceipt = createServerFn({ method: "POST" })
    ...
    if (!updated?.length)
        throw new Error("Comprovante não encontrado ou sem permissão para aprovar.");
...
export const rejectReceipt = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((data) => z.object({ receiptId: z.string().uuid(), reason: z.enum(["rejected", "duplicate"]).default("rejected"), note: z.string().optional() }).parse(data))
    .handler(async ({ data, context }) => {
    const { data: prev } = await context.supabase.from("receipts").select("status, profile_id, property_id, notes").eq("id", data.receiptId).single();
    const { data: updated, error } = await context.supabase
        .from("receipts")
    ...
    if (error)
        throw new Error(error.message);
    if (!updated?.length)
        throw new Error("Comprovante não encontrado ou sem permissão para alterar.");
    await logAudit(context.supabase, context.userId, {
...
export const archiveReceipt = createServerFn({ method: "POST" })
    ...
    if (error)
        throw new Error(error.message);
    if (!updated?.length)
        throw new Error("Comprovante não encontrado ou sem permissão para arquivar.");
    await logAudit(context.supabase, context.userId, {
...
export const bulkReceiptAction = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data, context }) => {
    const map = { approve: "approved", reject: "rejected", duplicate: "duplicate", archive: "archived" };
...
export const deleteReceipts = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((data) => z.object({ receiptIds: z.array(z.string().uuid()).min(1).max(500) }).parse(data))
    .handler(async ({ data, context }) => {
    ...
    const paths = Array.from(new Set(deleted.map((r) => r.safe_file_path).filter((p) => !!p)));
...
export const updateReceiptConference = createServerFn({ method: "POST" })
    ...
    if (Object.keys(diff).length === 0) {
        return { ok: true, receipt: existing, changed: [] };
    }
    ...
    await logAudit(supabase, userId, {
        ...
        profile_id: updated?.profile_id ?? existing.profile_id,
        property_id: updated?.property_id ?? existing.property_id,
        old_value: oldValues,
        new_value: diff,
    });
