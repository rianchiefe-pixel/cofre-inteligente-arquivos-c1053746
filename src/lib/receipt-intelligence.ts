 ...
 */
export function normalizePayeeName(name: string | null | undefined): string {
  if (!name) return "";
  return name
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
export async function getPayeeHistory(params: {
  ...
  const history = data.filter((r: any) => {
    if (taxId && r.recipient_tax_id === taxId) return true;
  ...
  history.forEach((r: any) => {
    ...
    if (r.profile_id) profiles[r.profile_id] = (profiles[r.profile_id] || 0) + 1;
    if (r.amount) values.push(Number(r.amount));
  });
