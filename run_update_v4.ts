...
async function main() {
  ...
  for (const row of csvData) {
    ...
    // CORREÇÃO: Remover TUDO que não é dígito para pegar os centavos
    const amountStr = row.Valor.replace(/[^\d]/g, '');
    ...
    // Busca exata por Valor, Perfil e (Data ou AuthCode)
    ...
    if (authCode) {
      // Prioridade absoluta para AuthCode
    ...
    if (matches && matches.length === 1) {
      ...
      if (filtered.length === 1) match = filtered[0];
          ...
          // Se ainda ambíguo, pega o primeiro se forem duplicatas exatas de valor/data/auth
          match = matches[0];
      }
    } else {
        // Tentar com margem de data se não encontrou por data exata (apenas se não houver authCode)
        if (!authCode) {
    ...
    if (match) {
      ...
      console.warn(`Não encontrado: ${row.Data} - ${recipientCsv} - ${row.Valor} (Centavos: ${amount})`);
      notFound++;
    }
  }
