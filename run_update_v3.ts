...
async function main() {
  ...
  for (const row of csvData) {
    ...
    // Buscar com margem de data ±2 dias
    const dateStart = new Date(date); dateStart.setDate(dateStart.getDate() - 2);
    const dateEnd = new Date(date); dateEnd.setDate(dateEnd.getDate() + 2);
    ...
    if (!matches || matches.length === 0) {
      ...
      let match = null;
      if (authCode) {
        match = matches.find(m => m.auth_code === authCode);
      }
      
      if (!match) {
        const normCsv = normalizeString(recipientCsv);
        const nameMatches = matches.filter(m => normalizeString(m.recipient_name || "").includes(normCsv.substring(0, 5)));
        if (nameMatches.length === 1) match = nameMatches[0];
        else if (nameMatches.length > 1) {
          // Desempate por data exata
          const exactDate = nameMatches.filter(m => m.payment_date === date.toISOString().split('T')[0]);
      ...
      } else if (matches.length > 0 && !match) {
        // Se ainda não encontrou e tem matches, pega o que tem nome mais parecido se não for ambíguo
        const normCsv = normalizeString(recipientCsv);
        const closest = matches.map(m => ({ m, score: normalizeString(m.recipient_name || "").includes(normCsv.substring(0, 3)) ? 1 : 0 }))
                               .filter(x => x.score > 0);
        if (closest.length === 1) {
             match = closest[0].m;
            ...
            console.warn(`Ambiguidade/Não resolvido: ${row.Data} - ${recipientCsv} - ${row.Valor}`);
            notFound++;
        }
      }
    }
